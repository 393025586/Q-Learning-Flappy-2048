class AI
        constructor: (@live_reward = 1, @dead_reward = -100, @gamma = 0.8) ->
                @Q = {}
                @dead_already = false
        ...
        initQ: (s) ->
                ...
                if not @Q[s[0]]?
                        @Q[s[0]] = {}
                if not @Q[s[0]][s[1]]?
                        @Q[s[0]][s[1]] = {jump: (if s[1] < 0 then -100 else 0), idle: 0}
                return @Q[s[0]][s[1]]
        ...
        play: (@game, @state, @ui) ->
                ...
                if @last_s? and @last_a?
                        @writeQ @last_s, @last_a, r + @gamma * Math.max(rewards_for_actions.jump, rewards_for_actions.idle)
